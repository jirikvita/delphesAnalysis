...
# support inclusion of only some bins as specified in usedbins, 1.9.2021

def fitSignalStrength(hsig, hbg, hdata, is1D, usedbins = [], step = 0.01, mumin = 0., mumax = 4.):
    ...
    if is1D:
        for i in range(1, hdata.GetNbinsX()+1):
            if len(usedbins) > 0 and usedbins[i-1] <= 0:
                continue
        ...
        for i in range(1, hdata.GetNbinsX()+1):
            for j in range(1, hdata.GetNbinsY()+1):
                if len(usedbins) > 0 and usedbins[i-1][j-1] <= 0:
                    continue

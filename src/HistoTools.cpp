...
// _______________________________________________________________________________
  ...
  TH1D* DivideHistos(TH1D* h1, TF1 *f2, std::string tag)
  {
    ...
    if (h1 && f2) {
      ...
      TH1D *div = (TH1D*) h1 -> Clone((tag + std::string(h1 -> GetName()) + std::string(f2 -> GetName())).c_str()); 
      div -> Divide(f2);
      return div;
    }
    return 0;
  ...
  TH1D* DivideHistos(TF1 *f2, TH1D* h1, std::string tag)
  {
    ...
    if (h1 && f2) {
      ...
      TH1D *div = (TH1D*) h1 -> Clone((tag + std::string(f2 -> GetName()) + std::string(h1 -> GetName())).c_str()); 
  ...
  TH1D* DivideHistos(TH1D* h1, TH1D *h2, std::string tag)
  {
    ...
    if (h1 && h2) {
      ...
      TH1D *div = (TH1D*) h1 -> Clone((tag + std::string(h1 -> GetName()) + std::string(h2 -> GetName())).c_str()); 
      div -> Divide(h2);
      return div;
    }
    return 0;
  ...
  // ___________________________________
  /*
  TH2D* DiagonaliseHistos(TH2D* h2, TH1D *fun, std::string name)
  {
    ...
    if (h2 && fun) {
      ...
      TH2D *div = (TH2D*) h2 -> Clone(name.c_str()); 
      div -> Reset();
  ...
  */
  ...
  int DivideByBinWidth(TH1D* &h)
  {
  if (h) {

  TH1D *h1 = (TH1D*) h -> Clone(Form("divided_%s", h -> GetName()));
  h1 -> Sumw2();
 ...
 // ________________________________________________________________________
  ...
  void SetHistoStyle(TH1D *h, int col, int width, int mark, float size, int rebin, 
 ...
 // ________________________________________________________________________
  ...
  int PrintBinContentTeX(int nHistos, TH1** h1, TString *Tags, bool *PrintErrorInsteadOfBinContent, TString outfilename, TString caption, TString label, 
			 ...
			 TH1D *ScaleHisto, TH1D *CentralHisto,
			 bool Symmetrise)
  ...
  // ___________________________________
  void AddErrorBarsRelative(TH1D *from, double CentralConstant, TH1D* to, TH1D *scale)
  {
  ...
  TH1D *MakeFitResidualsHisto(TH1D* h1, TF1 *fit, std::string tag, bool _relative,
			      float xmin, float xmax)
  {
    ...
    if (h1 && fit) {
      ...
      TH1D *res = (TH1D*) h1 -> Clone((tag + std::string(h1 -> GetName()) + std::string(fit -> GetName())).c_str()); 
      res -> Reset();
  ...
  TH1D *MakeFitResidualsProjection(TH1D* h1, TF1 *fit, std::string tag , bool _relative,
				   ...
				   float xmin, float xmax)
  {
	...
	x2 *= 0.9;
      ...
      TH1D *res =  new TH1D((tag + std::string(h1 -> GetName()) + std::string(fit -> GetName())).c_str(),
  ...
  TH2D *MergeEnsembleHistos(int Nesb, TH1D **hist, int nbinsy, std::string name, float maxmaxy, int ibin0)
  {
  ...
  {
    TH1D *histo = 0;
    if (graph && BinningHisto) {
      
      histo = (TH1D*)BinningHisto -> Clone(name.Data());
      histo -> Reset();
      double x, y, ex, ey;
  ...
  TH1D* MakeHalfMaxDiffHisto(std::vector<TH1D*> histos, std::string name)
  {
	...
	if (!histo) {
	  ...
	  histo = (TH1D*) histos[i] -> Clone(name.c_str());
	  histo -> Reset();
  ...
  TH1D* MakeSymErrorBand(TH1D *center, TH1D *Err, TString name, bool reset, double center_value)
  {
    ...
    TH1D *histo = 0;
    if (CloneFrom) {
      histo = (TH1D*) CloneFrom -> Clone(name);
      if (reset) {
	histo -> Reset();
      }
      if (Err) {
  ...
  TH1D *MakeDiagonalHisto(TH2D *h2, TString name) 
  {
    ...
    if (h2) {
      ...
      h = new TH1D(name, TString("Purity ") + h2 -> GetTitle(), h2->GetNbinsX(), binsx);
  ...
  TH1D *MakePoissonSmearedHisto(TH1D *h, TRandom3 *rand, std::string tag)
  {
    if (h) {
      TH1D* hsmear = (TH1D*) h -> Clone(Form("%s%s", h -> GetName(), tag.c_str()));
      hsmear -> Reset();
  ...
  TH1D* MakeConstHisto(TH1D *h, float CentralConst, float Const, std::string name, bool MultiplyByHisto)
  {
    TH1D* hnew = 0;
    if (h) {
      hnew = (TH1D*) h -> Clone(name.c_str());
      hnew -> Reset();
...
TH1D* GetHisto1DFromFile(TFile *file, std::string dirname, std::string hname)
{
  ...
  TH1D *h = 0;
  h = (TH1D*) file -> Get((dirname + hname).c_str());
  ...
  */
  ...
  if (!h) 
    cout << "ERROR: GetHisto1DFromFile::Failed getting histogram " << (dirname+hname).c_str() << endl;
...
TH2D* GetHisto2DFromFile(TFile *file, std::string dirname, std::string hname)
{
  ...
  TH2D *h = 0;
  h = (TH2D*) file -> Get((dirname + hname).c_str());
...
// ___________________________________________________________
  ...
  TH1D *TruncateBinsFromHisto(TH1D *hist, TString name, int binout1, int binout2)
  {
	...
	cout <<  binsxnew[i-1] << " " <<  binsxnew[i] << " "
      ...
      TH1D *histnew = new TH1D(name, hist -> GetTitle(), nbinsnew, binsxnew);  
      histnew -> Sumw2();
      for (int i = 1; i <= nbinsnew; ++i) {
  ...
  TGraphErrors* MakeGraphFromHisto(TH1D *histo, TString name, bool ignoreYerrors)
  {
  ...
  int MakeShapeAndAbsoluteErrorHistos(TH1D *central, TH1D *shifted, TH1D *&shape, TH1D *&absolute)
  {
    ...
    if (central and shifted) {
      ...
      shape = (TH1D*) shifted -> Clone(TString(shifted -> GetName()) + "_shape");
      double alpha =  central -> Integral("width") / shape -> Integral("width");
      shape -> Scale(alpha);
      absolute = (TH1D*) shifted -> Clone(TString(shifted -> GetName()) + "_absolute");
...
int AddHistosAver(TH1D *from, TH1D* to, double AverFactor)
{
